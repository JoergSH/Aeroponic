...
bool as7341_init() {
    // Haengt an Wire1 (I2C1, siehe pinout.h) statt am Standard-Wire (I2C0).
    initialized = as7341.begin(AS7341_I2CADDR_DEFAULT, &Wire1);
    if (!initialized) {
...
static void fill_invalid_data() {
    // Sensor nicht vorhanden/defekt: alles 0, keine erfundenen Werte. Gueltigkeit wird
    // primaer ueber as7341_is_ok() / RS485_MS2_REG_STATUS Bit1 signalisiert; gain=0xFF
    // (kein echter Gain-Index, gueltig sind nur 0-10) dient zusaetzlich als Sentinel.
    result.ppfd_dppfd = 0;
    result.ch_415nm   = 0;
    result.ch_445nm   = 0;
    result.ch_480nm   = 0;
