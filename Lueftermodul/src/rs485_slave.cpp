...
static void handle_frame() {
    ...
    if (rx_buf[0] != RS485_SLAVE_ADDR) return;   // fremde Adresse (anderes Geraet am Bus)
...
void rs485_slave_set_write_cb(rs485_slave_write_cb_t cb) { write_cb = cb; }
...
void rs485_slave_update(uint16_t reg, uint16_t value) {
    if (reg < RS485_NUM_REGS) regs[reg] = value;
}
